...
(function() {
  ...
  async function getSetting(key) {
    try {
      ...
      const result = await chrome.storage.local.get({ [storageKey]: DEFAULT_SETTINGS[key] });
      ...
      if (keysToCheckOld.length > 0) {
        try {
          ...
          if (Object.keys(toMigrate).length > 0) {
            ...
            console.log('[BetterE-class] Auto-migrated settings from sync:', Object.keys(toMigrate));
          }
        } catch (error) {
  ...
  async function migrateFromSync() {
    try {
      console.log('[BetterE-class] Migrating settings from sync to local...');
      ...
      if (Object.keys(oldSettings).length > 0) {
        ...
        if (Object.keys(newSettings).length > 0) {
          ...
          console.log('[BetterE-class] Migrated settings:', newSettings);
        }
      }
  ...
  async function migrateFromLocalStorage(oldKey, newKey, defaultValue = null) {
    try {
      ...
      if (oldData) {
        ...
        console.log(`[BetterE-class] Migrated ${oldKey} from localStorage to chrome.storage.local`);
